  ...
  <div>
    <v-tabs v-model="tab">
      <v-tooltip right>
        ...
        <span>Сохранить всенастройки</span>
      </v-tooltip>
    ...
    <v-tabs-items v-model="tab">
      ...
      <v-tab-item>
            ...
            <v-card-text>
              <div class="mx-auto  px-2 py-2" >          
                ...
                <v-dialog v-model="dialogSettings" max-width="80%" >
                  <atol-settings
                    v-on:selected="saveSettings(fiscalPrinterSettings)"
                  ></atol-settings>
                </v-dialog>
              </div>  
        ...
        <div v-if="fiscalRegisters.length">
          <v-row v-for="(fiscalRegister, key) in fiscalRegisters" :key="fiscalRegister.id">            
            <v-col cols="1"> {{ key + 1 }}
            </v-col>
            <v-col cols="2"> 
                        ...
                        <v-switch
                          v-model="selectedFR[key]"
                          inset
                        ></v-switch>
            </v-col>
            <v-col cols="3"> {{ fiscalRegister.model }}
            </v-col>
            <v-col cols="3"> {{ fiscalRegister.serial }}
            </v-col>
            <v-col cols="3">
              <v-tooltip bottom>
                <template v-slot:activator="{ on, attrs }">
                  <v-btn v-bind="attrs" v-on="on" icon  @click="deleteFiscalRegister(fiscalRegister._id)">  
                      <v-icon class="error--text">mdi-delete-outline</v-icon>           
                    </v-btn>
                </template>
    ...
    <!-- диалог изменения пользователя -->
                    ...
                    <v-card>
                      <v-card-title>
                        ...
                        <v-card-actions>                    
                          <v-btn @click="updateUser()" width="40%" height="50px" dark color="green lighten-2">
                            Изменить       
                          </v-btn>
                          <v-spacer></v-spacer>          
                        </v-card-actions>
